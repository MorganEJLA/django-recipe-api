...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_delete_other_users_recipe_error(self):
        ...
        new_user = create_user(email='user5@example.com',password='testpass123')
    ...
    def test_create_recipe_with_new_ingredients(self):
        ...
        payload = {
            ...
            'time_minutes': 22,
            'price': Decimal('5.00'),
            'ingredients': [{'name': 'Condensed Milk'}, {'name': 'Cocoa Powder'}],
        }
        res =self.client.post(RECIPES_URL, payload, format='json')
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        recipes = Recipe.objects.filter(user=self.user)
    ...
    def test_create_recipe_with_existing_ingredient(self):
        ...
        ingredient=Ingredient.objects.create(user=self.user, name='Condensed Milk')
        payload = {
            'title': 'Cocada Cremosa',
            'time_minutes': 22,
            'price': Decimal('5.00'),
            'ingredients': [{'name': 'Condensed Milk'}, {'name': 'Coconut'}],
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        recipes = Recipe.objects.filter(user=self.user)
    ...
    def test_filter_by_tags(self):
        ...
        r1 = create_recipe(user=self.user, title='Bombocado')
        r2 = create_recipe(user=self.user, title='Pastel de Nata')
        tag1 = Tag.objects.create(user=self.user, name='Brazil')
        tag2 = Tag.objects.create(user=self.user, name='Portugal')
        r1.tags.add(tag1)
        r2.tags.add(tag2)
        r3 = create_recipe(user=self.user, title='Cannoli')
    ...
    def test_filter_by_ingredients(self):
        ...
        r1 = create_recipe(user=self.user, title = 'Kladkakka')
        r2 = create_recipe(user=self.user, title = 'Malasadas')
        ...
        r3 = create_recipe(user=self.user, title = 'Quejinhos de Amendoa')
...
class ImageUploadTests(TestCase):
    """ Tests for uploading the image API """
